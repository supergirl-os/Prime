...
Init(page_id_t page_id, page_id_t parent_id) {
  ...
  // Set current size as 1 for the first invalid key
  SetSize(1);
  // set page id
  SetPageId(page_id);
  // set parent id
  ...
  // set max page size
  int size;
  size = (PAGE_SIZE - sizeof(BPlusTreeInternalPage))/
...
//INDEX_TEMPLATE_ARGUMENTS
template <typename KeyType, typename ValueType, typename KeyComparator>
ValueType BPlusTreeInternalPage<KeyType, ValueType, KeyComparator>::Lookup(const KeyType &key,
